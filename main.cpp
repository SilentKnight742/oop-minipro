...
{
    int n, flag = 0;
    ifstream bookf("books.txt");
    ofstream disf("dispose.txt", ios::app);
    ofstream tempf("temp.txt");
    cout << "Enter Book ID : ";
    cin >> n;
    while (bookf.read((char *)&l, sizeof(l)))
    {
        if (n == l.getID())
        {
            l.showBook();
            flag++;
            disf.write((char *)&l, sizeof(l));
        }
        else
        {
            tempf.write((char*)&l, sizeof(l));
        }
    }
    bookf.close();
    disf.close();
